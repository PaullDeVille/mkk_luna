...
class TestOrganizationsAPI:
    ...
    async def test_orgs_in_rectangular_area(
        ...
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 1  # Как минимум центральная организация
    ...
    async def test_orgs_in_rectangular_area_invalid_params(
        ...
        response = await client.get(
            ...
            params={
                "lat": 55.0,
                "lon": 37.0,
                "width_m": -100,  # Отрицательное значение
                "height_m": 100
            },
            headers=api_headers
        )
    ...
    async def test_orgs_in_rectangular_area_large_area(
        ...
        # Поиск в большой области (5000м x 5000м)
        response = await client.get(
            "/api/v1/organizations/geo/rectangular-area",
            params={
                "lat": 55.75,
        ...
        assert response.status_code == 200
        data = response.json()
        assert len(data) >= 3  # Должно найтись несколько организаций
